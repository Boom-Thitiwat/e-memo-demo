...
// ── Email via SMTP ตัวอย่าง (Vercel API route) ─────────────────────────────────
...
function printSystemPDF(memo, users) {
  ...
  html += '<span style="width:40px;height:40px;border-radius:8px;background:#D4AF37;color:#111;display:inline-flex;align-items:center;justify-content:center;font-weight:800;font-size:18px;flex-shrink:0;">E</span>';
...
function SettingsView({ notifyConfig, showToast, onOpenPdfTemplate }) {
  ...
  const CHANNELS = [
    { id:"email", icon:"✉", label:"อีเมล์ (SMTP ตัวอย่าง)", color:"#1E40AF",
      fields:[],
      guide:["ส่งจาก sender@example.com ผ่าน smtp.example.com:465 โดยอัตโนมัติ","ตั้งค่าตัวแปรใน Vercel Dashboard → Settings → Environment Variables:","SMTP_HOST = smtp.example.com  |  SMTP_PORT = 465","SMTP_USER = sender@example.com  |  SMTP_PASS = (รหัสผ่าน)","SMTP_FROM = \"E-Memo Demo\" <sender@example.com>","เปิด Toggle แล้ว Redeploy — ไม่ต้องตั้งค่าอะไรเพิ่มในหน้านี้"] },
        ...
        <div style={{width:"100%",maxWidth:720}}>
          ...
          <div ref={previewRef} style={{background:"#fff",borderRadius:4,boxShadow:"0 4px 40px rgba(0,0,0,.5)",padding:"32px 36px",position:"relative",minHeight:900,userSelect:"none"}}>
            ...
            <div style={{display:"flex",alignItems:"center",gap:14,borderBottom:"2px solid #1E3A5F",paddingBottom:10,marginBottom:18}}>
              <div style={{width:52,height:52,borderRadius:8,background:GOLD,color:BLACK,display:"flex",alignItems:"center",justifyContent:"center",fontSize:24,fontWeight:800,flexShrink:0}}>E</div>
              <div style={{flex:1}}>
      ...
      <div className="app-sidebar" style={{width:210,background:BLACK,color:"#fff"}}>
        <div style={{padding:"16px 16px 12px",borderBottom:"1px solid #222"}}>
          <div style={{display:"flex",alignItems:"center",gap:8}}>
            ...
            <div><div style={{fontSize:12,fontWeight:600,color:GOLD,letterSpacing:.3}}>E-Memo System</div><div style={{fontSize:9,color:"#555",lineHeight:1.3,marginTop:1}}>Demo Organization</div></div>
          </div>
        </div>
