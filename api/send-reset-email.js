...
// ส่ง reset password ผ่าน SMTP ตัวอย่าง (sender@example.com)
...
//   SMTP_PASS        = <password>
//   SMTP_FROM        = "E-Memo Demo" <sender@example.com>
//   FIREBASE_API_KEY = AIzaSy...  (Web API Key จาก Firebase Console → Project Settings)
//   APP_URL          = https://example.com/ememo-demo
...
export default async function handler(req, res) {
  ...
  // ── Step 2: ส่ง SMTP email จากเมลตัวอย่างพร้อมกัน ──────────────────────────
  if (process.env.SMTP_USER && process.env.SMTP_PASS) {
    const appUrl = process.env.APP_URL || "https://example.com/ememo-demo";
  ...
  <div style="border:1px solid #E5E7EB;border-top:3px solid #CC2229;padding:28px;border-radius:0 0 8px 8px;">
    ...
    try {
      ...
      await transporter.sendMail({
        from:    process.env.SMTP_FROM || `"E-Memo Demo" <${process.env.SMTP_USER}>`,
        to:      email,
        subject: subjectText,
        html,
